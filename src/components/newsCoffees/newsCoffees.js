				...
				{first4Coffees.map((coffee) => {
					return (
						<div className=' IndvCoffee border p-3 text-center rounded mt-8 mr-5 w-fit hover:bg-gray-100' key={coffee.id}>
							...
							<p  >{coffee.price},00€</p>
							<button  onClick={() => addToCartNoRepeat({name:coffee.name, image: coffee.image, price:coffee.price, bags:1})}  className="text-white bg-green-900 hover:bg-opacity-50 focus:ring-4 focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 p-1 m-3 mt-5">Añadir</button>
						</div>
					)
				})}
			</div>
